...
RegisterNetEvent('mdt:server:NewBulletin', function(title, info, time)
	...
	TriggerClientEvent('mdt:client:newBulletin', -1, src,
		{ id = newBulletin, title = title, info = info, time = time, author = PlayerData.CitizenId }, JobType)
end)
								...
								MySQL.insert(
									'INSERT INTO `mdt_impound` (`vehicleid`, `linkedreport`, `fee`, `time`) VALUES (:vehicleid, :linkedreport, :fee, :time)',
									{
										...
										local data = {
											...
											beingcollected = 0,
											vehicle = sentVehicle,
											officer = Player.PlayerData.charinfo.firstname ..
	...
	function(id, title, information, tags, officers, civilians, evidence, associated, time)
		...
		if Player then
			if GetJobType(Player.PlayerData.job.name) == 'police' then
				...
				elseif id > 0 then
					MySQL.Async.fetchAll('SELECT `author`, `title`, `details` FROM `mdt_incidents` WHERE `id` = @id',
		...
		if Player then
			MySQL.scalar("SELECT pfp FROM `mdt_data` WHERE cid=:id LIMIT 1", {
				...
				local Item = {
					...
					cid = Player.PlayerData.citizenid,
					name = '(' ..
						callsign .. ') ' .. Player.PlayerData.charinfo.firstname .. " " ..
						Player.PlayerData.charinfo.lastname,
					message = message,
					time = time,
					job = Player.PlayerData.job.name
	...
	if Player then
		if GetJobType(Player.PlayerData.job.name) == 'police' then
			if sentInfo and type(sentInfo) == 'table' then
				...
				if (plate and linkedreport and fee and time) then
					local vehicle = MySQL.query.await(
						"SELECT id, plate FROM `player_vehicles` WHERE plate=:plate LIMIT 1",
						{ plate = string.gsub(plate, "^%s*(.-)%s*$", "%1") })
					if vehicle and vehicle[1] then
...
RegisterNetEvent('mdt:server:removeImpound', function(plate, currentSelection)
	...
	if Player then
		if GetJobType(Player.PlayerData.job.name) == 'police' then
			local result = MySQL.single.await("SELECT id, vehicle FROM `player_vehicles` WHERE plate=:plate LIMIT 1",
				{ plate = string.gsub(plate, "^%s*(.-)%s*$", "%1") })
...
RegisterNetEvent('mdt:server:statusImpound', function(plate)
	...
	if Player then
		if GetJobType(Player.PlayerData.job.name) == 'police' then
			local vehicle = MySQL.query.await("SELECT id, plate FROM `player_vehicles` WHERE plate=:plate LIMIT 1",
				{ plate = string.gsub(plate, "^%s*(.-)%s*$", "%1") })
			if vehicle and vehicle[1] then
				local data = vehicle[1]
				local impoundinfo = MySQL.query.await("SELECT * FROM `mdt_impound` WHERE vehicleid=:vehicleid LIMIT 1",
					{ vehicleid = data['id'] })
...
function GetWarrantStatus(plate)
	local result = MySQL.query.await(
		"SELECT p.plate, p.citizenid, m.id FROM player_vehicles p INNER JOIN mdt_convictions m ON p.citizenid = m.cid WHERE m.warrant =1 AND p.plate =?",
		{ plate })
	if result and result[1] then
...
function GetVehicleOwner(plate)
	local result = MySQL.query.await('SELECT plate, citizenid, id FROM player_vehicles WHERE plate = @plate',
		{ ['@plate'] = plate })
	if result and result[1] then
		local citizenid = result[1]['citizenid']
		local Player = QBCore.Functions.GetPlayerByCitizenId(citizenid)
...
local function giveCitationItem(src, citizenId, fine, incidentId)
	...
	local OfficerFullName = '(' ..
		Officer.PlayerData.metadata.callsign ..
		') ' .. Officer.PlayerData.charinfo.firstname .. ' ' .. Officer.PlayerData.charinfo.lastname
	local info = {}
